...
$extra = Get-Content -Raw -Path (Join-Path $dir 'build\extra_tells.json') -Encoding UTF8 | ConvertFrom-Json
...
function Plats($d) {
  if ($d -eq 'lyrics') { return @('suno', 'udio') }
  elseif ($d -eq 'prose') { return @('chatgpt', 'gemini', 'claude') }
  else { return @('chatgpt', 'gemini', 'suno', 'udio') }
}
$seen = @{}
...
foreach ($t in $extra.templates) {
  ...
  $entries.Add([PSCustomObject]@{
    ...
    platforms = (Plats $t.domain); why = $t.why; example = ''; severity = $t.severity
    pattern = $t.pattern; flags = $flags; sources = @('extra-tells'); source = 'extra-tells'
  })
  $added.template++
}
foreach ($l in $extra.literals) {
  $k = $l.type + '|' + (Norm $l.term)
  if ($seen.ContainsKey($k)) { continue }
  $seen[$k] = $true
  $entries.Add([PSCustomObject]@{
    term = $l.term; type = $l.type; category = $l.category; domain = $l.domain
    platforms = (Plats $l.domain); why = $l.why; example = ''; severity = $l.severity
    sources = @('extra-tells'); source = 'extra-tells'
  })
  $added.literal++
}
foreach ($s in $extra.structures) {
  $k = 'structure|' + (Norm $s.term)
  if ($seen.ContainsKey($k)) { continue }
  $seen[$k] = $true
  $entries.Add([PSCustomObject]@{
    term = $s.term; type = 'structure'; category = $s.category; domain = $s.domain
    platforms = (Plats $s.domain); why = $s.why; example = ''; severity = $s.severity
    sources = @('extra-tells'); source = 'extra-tells'
  })
  $added.structure++
}
...
$data.meta.builtBy = '26-agent workflow + compass + pattern-detectors + extra-tells'
$data.entries = $all
($data | ConvertTo-Json -Depth 12) | Out-File -FilePath $jsonPath -Encoding utf8

Write-Output ("NEW added (after dedupe): templates {0}, literals {1}, structures {2} = {3}" -f $added.template, $added.literal, $added.structure, $totalAdded)
